	...
	function (search, record, render, file, Runtime, Log, common, xml, encode, email, https, url, _, moment) {
		...
		function createDealerFileToDownload(scriptContext) {
			...
			var Columns = [
				...
				search.createColumn({ name: "custbody_skn_bill_date", label: "bill_date" })
			];
			...
			for (var p = 0; p < data_bill.length; p++) {
				...
				//	log.debug("filtered_array",filtered_array)
				...
				//log.debug('Record Type', data_bill[p].type)
				if (data_bill[p].type == 'VendBill') {
					var type = 'BILL';
					var voucher_type = 'Bill';
				} else {
					var type = 'DEBIT NOTE';
					var voucher_type = 'Debit Note';
				}
				...
				//data_inv[j].quantity;
				//	log.debug('Bill_Date',data_bill[p].bill_date);

				fileContent += type + DELIMITER;
				fileContent += data_bill[p].tranid + DELIMITER;
				fileContent += data_bill[p].bill_date + DELIMITER;
				...
				fileContent += blank_field + DELIMITER;
				fileContent += data_bill[p].vendor_code + DELIMITER;
		...
		function writeFileIntoFileCabinet(fileContent, report_type, userObj, gstNumber) {
			...
			log.debug("writeFileIntoFileCabinet", "writeFileIntoFileCabinet");
		...
		function SendFileToClearTax(file_id, fileName, report_type, record_id) {
			...
			log.debug('headersobj', headersobj);
			log.debug('headersobj', typeof headersobj);
		...
		function getFileName(gstNumber) {
			log.debug('getFileName', "getFileName");
			log.debug('GST number', gstNumber);
			var formattedDate = moment().format('YYMMDD');
			return 'GSTR' + gstNumber + ' _PURCHASE_REPORT' + formattedDate + '.csv';
