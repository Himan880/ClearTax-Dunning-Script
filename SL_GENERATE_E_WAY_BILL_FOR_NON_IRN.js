	...
	function onRequest(context) {
		try {
          ...
          log.debug*('ewaystatus',ewaystatus);
            ...
            if(ewaystatus == 4 &&  (ewayno == null || ewayno == 'null' || ewayno == "") )
            {
                        ...
                        if (response.body) {
                            ...
                            log.debug('GenerateEWayBill responsebody : ', responsebody);
                                ...
                                log.debug('responsebody.EwbNo ', responsebody.EwbNo);
								 ...
								 let ewbbilldateget = responsebody.EwbDt
				// Input date-time string
const inputDateTime = ewbbilldateget;

// Convert to a Date object
const date = new Date(inputDateTime.replace(' ', 'T'));
...
const hours12 = hours24 % 12 || 12; // Convert to 12-hour format, handle midnight as 12

// Construct the formatted string
const formattedDateTime = `${day}/${month}/${year} ${hours12}:${minutes}:${seconds} ${period}`;  
								  
			  var EwbDt = format.parse({
                                            value: formattedDateTime,
                                            type: format.Type.DATETIMETZ
                                });
                                  log.debug('EwbDt', EwbDt);					  
							   ...
							   let ewbbilldateValid = responsebody.EwbValidTill
				// Input date-time string
const inputDateTime12 = ewbbilldateValid;

// Convert to a Date object
const date12 = new Date(inputDateTime12.replace(' ', 'T'));

// Helper function to format numbers with leading zeros
const pad12 = (number) => number.toString().padStart(2, '0');

// Convert to desired format
const day12 = pad12(date12.getDate());
const month12 = pad(date12.getMonth() + 1); // Months are zero-based
const year12 = date12.getFullYear();
const hours2412 = date12.getHours();
const minutes12 = pad12(date12.getMinutes());
const seconds12 = pad12(date12.getSeconds());

// Determine AM/PM
const period12 = hours2412 >= 12 ? 'pm' : 'am';
const hours1212 = hours2412 % 12 || 12; // Convert to 12-hour format, handle midnight as 12

// Construct the formatted string
const formattedDateTime12 = `${day12}/${month12}/${year12} ${hours1212}:${minutes12}:${seconds12} ${period12}`;  

 var EwbTill = format.parse({
                                            value: formattedDateTime12,
                                            type: format.Type.DATETIMETZ
                                });
                                
                               log.debug('EwbTill', EwbTill);
